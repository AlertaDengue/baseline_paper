# Auxiliar functions for epibands package
# Leo Bastos
...
forecasting.inla <- function(data.inla,   # dados - Data containing columns (cases, week, year, target)
                             #Year2forecast = 2023,
                             MC = FALSE, M = 2000,
  ...
  out$pred = data.inla %>% ungroup() |> filter(target == T) %>% select(week, year) |> 
